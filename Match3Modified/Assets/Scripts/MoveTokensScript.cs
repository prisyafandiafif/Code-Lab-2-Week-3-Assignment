...
{
	public static MoveTokensScript instance; //ADDED this
   ...
   // protected MatchManagerScript matchManager; // A reference to the MatchManagerScript component on this script's gameObject.
    ...
    [SerializeField] public bool move = false; //Determines whether or not the selected tokens are moving.
    ...
    [SerializeField] Vector2 exchangeGridPos1; //The position of exchangeToken1 in the scene.
    [SerializeField] Vector2 exchangeGridPos2; //The position of exchangeToken2 in the scene.
    ...
    public virtual void Update()
    {
        ...
        if (move) //If the tokens are moving:
        {
            ...
            if (lerpPercent >= 1f) //If the lerpPercentage exceeds 1f:
            {
                lerpPercent = 1f; //...force it back to 1f.
            }
    ...
    public virtual void ExchangeTokens()
    {
        ...
        */
        Vector3 startPos = GameManagerScript.instance.GetWorldPositionFromGridPosition((int)exchangeGridPos1.x, (int)exchangeGridPos1.y);
        Vector3 endPos = GameManagerScript.instance.GetWorldPositionFromGridPosition((int)exchangeGridPos2.x, (int)exchangeGridPos2.y);
        ...
        */
        ...
        if (lerpPercent == 1f) //When the movement animation is complete:
        {
            /*
            The next two lines simply swap exchangeToken1 and exchangeToken2's positions in the grid array of the gameManager script.
            */
            GameManagerScript.instance.gridArray[(int)exchangeGridPos2.x, (int)exchangeGridPos2.y] = exchangeToken1;
            GameManagerScript.instance.gridArray[(int)exchangeGridPos1.x, (int)exchangeGridPos1.y] = exchangeToken2;

            if (!MatchManagerScript.instance.GridHasMatch() && userSwap) //If there's no match and the user tokens have been swapped:
            {
                SetupTokenExchange(exchangeToken1, exchangeGridPos2, exchangeToken2, exchangeGridPos1, false); //...reverse the swap. (cancel it)
            }
            else // Otherwise: 
			{ 
                                    ...
                                    GameObject token)
    {
        ...
        The positions are converted into grid positions by gameManager.GetWorldPositionFromGridPosition.
        */
        Vector3 startPos = GameManagerScript.instance.GetWorldPositionFromGridPosition(startGridX, startGridY);
        Vector3 endPos = GameManagerScript.instance.GetWorldPositionFromGridPosition(endGridX, endGridY);

        Vector3 pos = Vector3.Lerp(startPos, endPos, lerpPercent);

        token.transform.position = pos;

        if (lerpPercent == 1f) //If the lerp percentage is complete
        {
            /*
            Swap the valid token with the empty space.
            */
            GameManagerScript.instance.gridArray[endGridX, endGridY] = token; 
            GameManagerScript.instance.gridArray[startGridX, startGridY] = null;

			//ADDED THIS 
			move = false;

			Debug.Log("Finished moving token downward! : " + startGridX + "," + startGridY + " and " + endGridX + "," + endGridY);
    ...
    public virtual bool MoveTokensToFillEmptySpaces()
    {
        ...
        for (int x = 0; x < GameManagerScript.instance.gridWidth; x++) //For the length of the grid...
        {
            for (int y = 1; y < GameManagerScript.instance.gridHeight; y++) //...and for the height of the grid...
            {
				//Debug.Log("Why");

                if (GameManagerScript.instance.gridArray[x, y - 1] == null && GameManagerScript.instance.gridArray[x, y] != null) //If we find an empty space:
                {
                    for (int pos = y; pos < GameManagerScript.instance.gridHeight; pos++) //...using the y position as a reference...
                    {
						Debug.Log("Why");

                        GameObject token = GameManagerScript.instance.gridArray[x, pos]; //...grab a token on top of the empty space.

                        if (token != null) //If the token that we've just grabbed is a valid token (not empty)...
                        {
							Debug.Log("Okay: " + x + "," + pos);
		...
		//ADDED THIS (COMMENT THIS)
        /*if (lerpPercent == 1f) //If the lerp percent is 1
        {
            move = false;
        }*/
